...
class CartPurchaseModel {
    ...
    public function ViewPurchaseSale($username){
        $data = mysqli_query($this->server->mysql, "SELECT * FROM cart_purchase WHERE session_cart = '$username'");
    ...
    public function Add($id_item, $username){
        ...
        $cek_cart = mysqli_query($this->server->mysql, "SELECT * FROM cart_purchase WHERE id_cart = '$id_cart' AND session_cart = '$username'");
        $row = mysqli_num_rows($cek_cart);

        if($row == 1){
            return $this->msg->Warning('Data dengan nama barang '.$name_item_cart.' sudah ditambahkan di pembelian');
        }

        $insert = mysqli_query($this->server->mysql, "INSERT INTO cart_purchase (id, id_cart, name_cart, size_cart, size_type_cart, quantity_cart, unit_type_cart, purchase_price_cart,
    ...
    public function Delete($id){
        ...
        $delete = mysqli_query($this->server->mysql, "DELETE FROM cart_purchase WHERE id_cart = '$id'");
